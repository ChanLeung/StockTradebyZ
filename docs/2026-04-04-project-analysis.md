...
- 先按当前组合总权益的 `1/10` 计算单票目标预算
- 再按 `100` 股一手向下取整
- 同时受可用现金约束，保证能买得进

卖出则按真实持仓股数执行，不再写死固定数量。
这仍然不是完整的实盘级仓位引擎，但已经比早期“固定 100 股”的研究骨架更接近真实交易。
...
3. 把“同指数 / 同行业上限、WATCH 可替换、留现金原因”再往实盘细则继续收敛
4. 继续收敛旧兼容命名和旧配置文件，逐步减少 `gemini_*` 别名暴露
5. 视需要把买卖 reviewer 的 provider 封装进一步统一
